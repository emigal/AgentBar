...
struct KeyCombo: Equatable {
    ...
    // ⌘M globally: as a Carbon hotkey it wins over app menus, so it shadows the
    // system Minimize shortcut while AgentBar runs (Minimize is also remapped
    // away via NSUserKeyEquivalents so a stopped AgentBar doesn't resurrect it).
    static let defaultMenu = KeyCombo(keyCode: UInt32(kVK_ANSI_M),
                                      carbonModifiers: UInt32(cmdKey), display: "⌘M")
